...
export default async function OmPage() {
  ...
  const portraitImage = d.aboutImage ?? "/placeholder.svg";
  ...
  const quote = d.quote ?? "«Et godt hjem er ikke overfylt av ting. Det er fylt av riktige valg.»";
    ...
    <div className="bg-paper pt-[70px]">
      ...
      <section className="flex flex-col lg:flex-row items-center lg:items-start gap-10 lg:gap-24 px-10 sm:px-16 py-16">
        <div className="relative w-full lg:w-[460px] aspect-[460/552] lg:h-[552px] shrink-0">
          <Image src={portraitImage} alt="Martine Gullord Engebråten" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 460px" />
        </div>
        <div className="flex flex-col gap-5 max-w-[500px]">
          ...
          <p className="italic text-subheading text-ink/85 pt-2">{quote}</p>
        </div>
      </section>

      {/* Gallery */}
      <section className="flex flex-col sm:flex-row gap-4 px-10 sm:px-16 py-16">
        {galleryLayout.map((column, ci) => (
          <div key={ci} className="flex flex-1 flex-col gap-4">
            {column.map((tile, ti) => {
              ...
              imageCursor += 1;
              return (
                <div key={ti} className={`relative w-full ${tile.aspect}`}>
                  <Image
                    src={src}
