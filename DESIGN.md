...
|------|-------|-------|------|
| Ink | `#544f4d` | `--color-press-black` | Primary text, 1px hairlines, icon strokes, outlined button borders. The structural ink of the entire system — every border, every label, every headline resolves to this single value |
| Paper | `#f7f4ee` | `--color-gallery-white` | Page canvas, card surfaces, button text on dark surfaces, inverse overlay backgrounds. The unbroken ground on which every photograph sits |
| Header Greige | `#e1ded8` | `--color-header-greige` | Header/nav surface once scrolled past the hero — sits between Ink and Paper in value |
| Plate Gray | `#b8b8b8` | `--color-plate-gray` | Subtle disabled state, placeholder fill, secondary metadata. Reads as 10.6:1 on ink (safe for muted labels) but only 2.0:1 on paper — never use as body text on the default canvas |
...
Full-width, position-absolute over hero, no background fill. Left cluster: nav links (About, Journal, Inspiration, Design) in Inter 16px weight 400 #544f4d with 20px horizontal spacing. Center: wordmark 'Stykka' in Inter 22-24px weight 500 #544f4d. Right cluster: language selector and 'Start building' outlined button. 20px vertical padding. No border, no shadow, no background.

### Outlined CTA Button
**Role:** The only button variant on the site — used for every actionable element

Inter 16px weight 500 #544f4d, 1px solid #544f4d border, 8px radius, padding 10px 18px (approx). Transparent fill so it sits cleanly over photography. Hover inverts: fill becomes #544f4d, text becomes #f7f4ee. No drop-shadow, no scale transform. The 8px radius is deliberately soft but not pill — it reads as pressed metal, not a tag.
...
Inter 14px weight 500 #f7f4ee (sits over photography), letter-spacing +0.29px, uppercase. Used in the hero and as the opener of every editorial section — it is the system's only repeated chrome. Color flips to #544f4d on white backgrounds.

### Display Headline
**Role:** Primary editorial statement on hero and section openers

Inter 46px weight 400 #f7f4ee on hero / #544f4d on white sections, line-height 1.05, letter-spacing -1.93px. Weight 400 (not 700) is the signature — the headline never shouts. It is set tight enough to feel architectural, not typeset.

### Feature Grid Card
**Role:** 4-column editorial card for product attributes

Image on top (full-bleed, hard 0px corners, 4:3 or 3:4 aspect), 10px row gap to title, title in Inter 14-16px weight 500 uppercase #544f4d letter-spacing +0.29px, 10px gap to body, body in Inter 16px weight 400 #544f4d line-height 1.50. No card surface, no border, no shadow — the image edge IS the card edge. 4 columns on desktop, collapses to 2 then 1.

### Language Selector
**Role:** Locale switcher in nav

Globe icon (16px stroke, 1px #544f4d) + 'English' in Inter 16px weight 400 + small chevron. No background, no border. Sits inline with other nav elements with 20px gap.
...
Inter 16px weight 400 #544f4d, 1px underline offset 3px, underline color #544f4d. No color shift, no bold — the link is identified by the underline alone, which keeps the editorial line unbroken.

### Footer Block
**Role:** Closing black band

Full-width #544f4d background, text in #f7f4ee. Inter 14-16px weight 400. Same 20px internal padding rhythm as the nav. Minimal: a wordmark, a link list, and a copyright line in system-ui 12px. No social icons, no newsletter form — the system stays austere to the end.

### Monospace Spec Callout
**Role:** Editorial accent line for a material or dimension

Azeret Mono 18px weight 400 #544f4d, line-height 1.00, letter-spacing -0.18px. Single line, full-bleed across the content column. Breaks the Inter rhythm deliberately — use for a quote, a dimension, or a builder's note. Never more than one per page.

## Do's and Don'ts

### Do
- Use #544f4d for every text element and every 1px border — never introduce a second ink color, the system is intentionally monochromatic
...
- Use the outlined CTA (1px #544f4d border, 8px radius, transparent fill) for every action — there is no filled or ghost variant, this is the only button
...
- Never place body text directly on a full-color photograph without a tonal treatment — the hero overlay uses #f7f4ee text, and any text on a photograph must be either pure white or pure black to survive the contrast

## Surfaces

| Level | Name | Value | Purpose |
|-------|------|-------|---------|
| 1 | Canvas | `#f7f4ee` | Default page background and image-negative space between sections |
| 2 | Ink | `#544f4d` | Reserved as a contrast surface for inverse buttons, footer block, or full-bleed black sections — not used as page background |

## Elevation

Zero shadow vocabulary. Structure is communicated entirely through 1px hairline borders in #544f4d and whitespace. Cards, sections, and feature blocks float by adjacency, not by depth. Adding a drop-shadow breaks the editorial flatness of the system — if you need to group something, draw a border instead.
...
- text: #544f4d
- background: #f7f4ee
- border: #544f4d (1px hairlines only)
...
1. HERO SECTION
Full-viewport image (100vw × 100vh) with no overlay or gradient. Lower-left text block on a 10px grid, padded 40px from the viewport edge. Small-caps label: Inter 14px weight 500, #f7f4ee, uppercase, letter-spacing +0.29px, content 'EXPLORE KITCHENS'. Display headline below: Inter 46px weight 400, #f7f4ee, line-height 1.05, letter-spacing -1.93px, content 'Built to last'. Outlined button below: Inter 16px weight 500, 1px solid #544f4d border (visible as a cutout or re-tinted to #f7f4ee when over dark photography), 8px radius, padding 10px 18px, label 'Start building'.

2. FEATURE GRID CARD
Image (4:3 aspect, 0px corner radius) on top. 10px row gap. Title: Inter 14px weight 500, #544f4d, uppercase, letter-spacing +0.29px. 10px gap. Body: Inter 16px weight 400, #544f4d, line-height 1.50, max-width 32ch. No card surface, no border, no shadow. Arranged in a 4-column grid with 20px column gaps, 40px row gaps.

3. TRANSPARENT TOP NAV
No distinct primary action color was observed; use the extracted neutral button treatments instead of inventing a filled CTA color.

4. SECTION HEADER
Stack on a centered 10px grid. Tracked label: Inter 14px weight 500, #544f4d, uppercase, letter-spacing +0.29px. 20px gap. Headline: Inter 30px weight 400, #544f4d, line-height 1.10, letter-spacing -1.20px. 20px gap. Optional supporting paragraph: Inter 16px weight 400, #544f4d, line-height 1.50, max-width 60ch.

5. FOOTER BLOCK
Full-width #544f4d background. 20px internal padding. Inter 14-16px weight 400, #f7f4ee. Single wordmark left, link list center, copyright in system-ui 12px, #f7f4ee right. No social icons, no newsletter form.
...
:root {
  /* Colors */
  --color-press-black: #544f4d;
  --color-gallery-white: #f7f4ee;
